  ...
  try {
    ...
    if (url.pathname === "/api/db/init" && method === "POST") {
      try {
        ...
        return jsonResponse({
          message: "Database schemas and relations initialized successfully!",
        });
      } catch (err: any) {
        return jsonResponse(
          { error: "Failed to initialize DB schemas", details: err.message },
          500,
        );
      }
    }
    ...
    if (url.pathname === "/api/state" && method === "GET") {
      try {
        ...
        return jsonResponse(
          { error: "Failed to fetch state", details: err.message },
          500,
        );
      }
    }
    ...
    const matchHabitId = url.pathname.match(
      /^\/api\/habits\/([a-zA-Z0-9_-]+)$/,
    );
    if (matchHabitId) {
    ...
    const matchTimeBlockId = url.pathname.match(
      /^\/api\/time-blocks\/([a-zA-Z0-9_-]+)$/,
    );
    if (matchTimeBlockId) {
    ...
    const matchMicroTaskId = url.pathname.match(
      /^\/api\/micro-tasks\/([a-zA-Z0-9_-]+)$/,
    );
    if (matchMicroTaskId) {
      ...
      if (method === "PUT") {
        ...
        const updated = await sql`
            UPDATE micro_tasks
            SET completed = CASE WHEN ${body.completed !== undefined} THEN ${body.completed}::boolean ELSE completed END,
                title = COALESCE(${body.title ?? null}, title),
    ...
    if (url.pathname === "/api/pillars" && method === "GET") {
      ...
      for (const p of pillars) {
        const milestones =
          await sql`SELECT id, year_horizon as year, quarter, title, completed FROM pillar_milestones WHERE pillar_id = ${p.id}`;
    ...
    const matchPillarMilestonesPost = url.pathname.match(
      /^\/api\/pillars\/([a-zA-Z0-9_-]+)\/milestones$/,
    );
    ...
    const matchMilestoneId = url.pathname.match(
      /^\/api\/pillars\/milestones\/([a-zA-Z0-9_-]+)$/,
    );
    if (matchMilestoneId) {
    ...
    if (url.pathname === "/api/intention" && method === "POST") {
      const body = await req.json();
      await sql`
          INSERT INTO daily_intentions (intention)
          VALUES (${body.intention});
        `;
    ...
    const matchTimelineId = url.pathname.match(
      /^\/api\/timeline\/([a-zA-Z0-9_-]+)$/,
    );
    if (matchTimelineId) {
...
if (typeof Bun !== "undefined") {
  ...
  console.log(
    `🚀 FocusOS Bun Backend running at http://localhost:${server.port}`,
  );
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      res.statusCode = 500;
      res.end(
        JSON.stringify({ error: "Node Server Error", details: e.message }),
      );
    }
  });
